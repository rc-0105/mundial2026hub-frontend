...
@Component({
              ...
              @for (e of p.eventos; track e.idEvento) {
                <div class="tl-event" [class]="'tl-' + e.equipo.toLowerCase()">
                  ...
                  <div class="tl-content">
                    ...
                    <span class="tl-equipo">{{ e.equipo === 'LOCAL' ? p.seleccionLocal.nombre : p.seleccionVisitante.nombre }}</span>
                  </div>
                </div>
              }
            </div>
          ...
          <div class="live-section">
            ...
            <div style="display: flex; align-items: center; gap: 1rem;">
              ...
              <div style="flex: 1; height: 10px; background: var(--gray-100); border-radius: 5px; overflow: hidden;">
                <div style="height: 100%; width: {{ p.posesionLocal }}%; background: var(--primary); border-radius: 5px; transition: width 0.5s;"></div>
              </div>
              <span style="font-size: 0.85rem; color: var(--gray-400); min-width: 4rem; text-align: center;">Posesión</span>
              <div style="flex: 1; height: 10px; background: var(--gray-100); border-radius: 5px; overflow: hidden;">
                <div style="height: 100%; width: {{ p.posesionVisitante }}%; background: #dc2626; border-radius: 5px; float: right; transition: width 0.5s;"></div>
              </div>
  ...
  styles: [`
    ...
    .tl-dot {
      position: absolute; left: -1.15rem; top: 0.85rem;
...
export class PartidoEnVivoComponent implements OnInit, OnDestroy {
  ...
  private cargarEventos(id: number): void {
    this.partidosService.getEventosPartido(id).subscribe({
      next: res => {
